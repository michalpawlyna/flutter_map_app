...
class FirestoreService {
  ...
  Stream<int> getPlaceLikedCountStream(String placeId) {
    return _db.collection('places').doc(placeId).snapshots().map((doc) {
      final data = doc.data() ?? <String, dynamic>{};
      return (data['likedCount'] as num?)?.toInt() ?? 0;
    });
  }
  ...
  Future<List<Achievement>> addPlaceToFavourites(
    String uid,
    String placeId,
  ) async {
    ...
    debugPrint(
      '[FirestoreService] addPlaceToFavourites called for user: $uid, placeId: $placeId',
    );
    ...
    debugPrint(
      '[FirestoreService] Current liked count: ${currentUser.totalPlacesLiked}, New count: $newLikedCount',
    );
    ...
    debugPrint(
      '[FirestoreService] Updated totalPlacesLiked to $newLikedCount and incremented place likedCount',
    );

    final newlyUnlocked = <Achievement>[];
    try {
      debugPrint(
        '[FirestoreService] Fetching achievements with type: ${AchievementType.likePlace.asString}',
      );
      ...
      debugPrint(
        '[FirestoreService] Found ${achQuery.docs.length} achievements with type likePlace',
      );

      for (final achDoc in achQuery.docs) {
        final achievement = Achievement.fromSnapshot(achDoc);
        final alreadyUnlocked =
            currentUser.achievementsSummary[achievement.id] == true;

        debugPrint(
          '[FirestoreService] Checking achievement: ${achievement.title} (${achievement.id})',
        );
        debugPrint('[FirestoreService]   Already unlocked: $alreadyUnlocked');
        debugPrint(
          '[FirestoreService]   Target: ${achievement.criteria['target']}, Progress: $newLikedCount',
        );
        ...
        if (target > 0 && newLikedCount >= target) {
          ...
          debugPrint(
            '[FirestoreService]   -> Not yet reached (target: $target, progress: $newLikedCount)',
          );
        }
      }

      debugPrint(
        '[FirestoreService] Total unlocked achievements: ${newlyUnlocked.length}',
      );
    } catch (e) {
      debugPrint(
        '[FirestoreService] Error checking place like achievements: $e',
      );
    }
  ...
  Future<PlaceVisitResult> reportPlaceVisit({
    ...
    final unlockedAchievements = await _achievementService
        .checkAndGrantAchievements(updatedUser, AchievementType.visit);
  ...
  Future<void> updateUserEquippedAchievements(
    String uid,
  ...
  Future<List<Achievement>> reportRouteCreation(String uid) async {
    ...
    if (!userSnap.exists) {
      ...
      await userRef.set({'totalRoutesCreated': 1}, SetOptions(merge: true));
      return [];
    }
    ...
    debugPrint(
      '[FirestoreService] Current route count: ${currentUser.totalRoutesCreated}, New count: $newProgress',
    );
    ...
    try {
      debugPrint(
        '[FirestoreService] METHOD 1: Fetching achievements with type: ${AchievementType.createRoute.asString}',
      );
      ...
      debugPrint(
        '[FirestoreService] Found ${achQuery.docs.length} achievements with type createRoute',
      );

      if (achQuery.docs.isEmpty) {
        debugPrint(
          '[FirestoreService] WARNING: No achievements found for type createRoute!',
        );
        ...
        for (final ach in allAchs.docs) {
          final data = ach.data();
          debugPrint(
            '[FirestoreService]   - ${ach.id}: type=${data['type']}, title=${data['title']}',
          );
        }
      }
      ...
      for (final achDoc in achQuery.docs) {
        ...
        debugPrint('[FirestoreService]   Already unlocked: $alreadyUnlocked');
        debugPrint(
          '[FirestoreService]   Target: ${achievement.criteria['target']}, Progress: $newProgress',
        );
        ...
        if (target > 0 && newProgress >= target) {
          ...
          debugPrint(
            '[FirestoreService]   -> Not yet reached (target: $target, progress: $newProgress)',
          );
        }
      }

      debugPrint(
        '[FirestoreService] METHOD 1: Total unlocked achievements: ${newlyUnlocked.length}',
      );
    } catch (e, st) {
    ...
    if (newlyUnlocked.isEmpty) {
      debugPrint(
        '[FirestoreService] METHOD 2 FALLBACK: Checking all achievements',
      );
      try {
        final allAchs = await _db.collection('achievements').get();
        for (final achDoc in allAchs.docs) {
          ...
          if (target > 0 && newProgress >= target) {
            debugPrint(
              '[FirestoreService] FALLBACK: Unlocked ${achievement.title}',
            );
            newlyUnlocked.add(achievement);
        ...
        debugPrint(
          '[FirestoreService] METHOD 2: Found ${newlyUnlocked.length} unlocked achievements',
        );
      } catch (e) {
